...
function TableEditor() {
    ...
    function titleChange({column, title} : {column: ColumnField, title: string}){
      column.name = title;
    }
                  ...
                  <DialogContent>
                      <DialogHeader>
                          ...
                          <DialogHeader>
                            ...
                            <DialogDescription>Enter an optional name, datatype, then rearrange the order.</DialogDescription>
                          </DialogHeader>
                      </DialogHeader>
                      <div className="overflow-hidden">
                          <div className="columnAdjuster">
                            ...
                            <div className="mb-2 flex items-center">
                              <Button onClick={addColumnField} className="shrink-0 bg-sky-300 text-black hover:bg-sky-400 h-10 w-10">
                              </Button>
                            ...
                            <div className="columnList">
                              {colArray.map((columnField, index) => (
                                <div className="flex justify-between mb-2" key={index}>
                                  <input
                                    type="text"
                                    onChange={(e) => (titleChange({column : columnField, title : e.target.value}))}
                                    placeholder="Enter the title here..."
                                    className="bg-neutral-200"
                                  />
                                  <Select defaultValue={"String"} onValueChange={(e) => (typeChange({column : columnField, type: e}))}>
                                  ...
                                  <Button onClick={() => removeColumnField({i : index})}>Delete</Button>
                                </div>
                              ))}
                            </div>
                          </div>
                          <DialogFooter className="mt-6">
                            ...
                            <Button onClick={addItem} className="shrink-0 bg-sky-300 text-black hover:bg-sky-400">Confirm</Button>
                          </DialogFooter>
                      </div>
                  </DialogContent>
                </Dialog>
